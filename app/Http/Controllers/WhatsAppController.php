...
class WhatsAppController extends Controller
{
    ...
    public function updateWhatsAppNumber(Request $request)
    {
        ...
        // Validate WhatsApp number (permite espacios; se limpian antes de guardar)
        $validated = $request->validate([
            ...
            'whatsapp_visible' => 'sometimes|boolean',
        ], [
            'whatsapp_number.regex' => 'Por favor ingresa un número de WhatsApp válido (ej: +591 700 12345)',
        ]);

        $validated['whatsapp_number'] = isset($validated['whatsapp_number'])
            ? preg_replace('/\s+/', '', $validated['whatsapp_number'])
            : null;

        // Only allow visible if user is verified and has a number
        if (($validated['whatsapp_visible'] ?? false) && (!$user->isVerified() || !$validated['whatsapp_number'])) {
    ...
    public function getAgentContact($propertyId)
    {
        ...
        $canContact = $agent->hasVisibleWhatsApp();
        return response()->json([
            'name' => $agent->name,
            'whatsapp_number' => $canContact ? $agent->whatsapp_number : null,
